...
const MyVideosMain = () => {
  ...
  const submitFormHandler = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/v1/videos/uploadVieo`, formData, {                withCredentials: true,
  ...
  return (
    <div className='text-white    overflow-auto'>
      ...
      {currentOpened == 1 && (
        <div className='grid md:grid-cols-2 lg:grid-cols-2 mt-2  '>
          {videos.map((video) => (
            <div key={video._id}
              className='flex flex-col lg:flex-row md:flex-row p-2  hover:bg-gray-800  duration-200 rounded-lg 
               overflow-hidden hover:cursor-pointer sm:w-fit w-full lg:w-fit'
            ...
            >
              <div className='relative'>
                <img
                  src={video.thumbnail}
                  alt={video.title}
                  className=' md:w-100 lg:w-100 aspect-video object-cover rounded'
                />
                <span className='absolute bottom-2 right-2 bg-[#2222] bg-opacity-70 px-2 font-bold rounded text-xs text-white'>
                  {Math.floor(video.duration)}s
                </span>
              </div>
              <div className=' md:pl-3 lg:pl-3 flex gap-5'>
                <div className='flex gap-2 w-60'>
                  <div>
                    <h3 className='text-white font-medium lg:text-2xl'>{shortDetails(video.title, 50)}</h3>
